...
try:
    ...
    LASSO_PARAMS = {'alpha': 0.001, 'max_iter': 10000}
    ELASTIC_PARAMS = {'alpha': 0.01, 'l1_ratio': 0.5, 'max_iter': 10000}
    RF_PARAMS = {'n_estimators': 100, 'max_depth': 10, 'min_samples_split': 20,
                 'min_samples_leaf': 10, 'max_features': 'sqrt', 'random_state': 42, 'n_jobs': -1}
    GB_PARAMS = {'n_estimators': 100, 'max_depth': 5, 'learning_rate': 0.1,
                 'min_samples_split': 20, 'min_samples_leaf': 10, 'subsample': 0.8, 'random_state': 42}
    ENSEMBLE_WEIGHTS = {'ridge': 0.15, 'lasso': 0.15, 'elastic': 0.15, 'rf': 0.25, 'gb': 0.30}
    ...
    def get_simulation_file():
        return f"L10_temp_optimized_{INVESTMENT_HORIZON.lower()}.pkl"
...
number_of_periods_per_year = get_number_of_periods_per_year()
simulation_file = get_simulation_file()

# 获取因子列表
all_factors = get_all_factors()
all_factors = all_factors.loc[[a in FACTOR_CATEGORIES 
...
def cal_portfolio_vwap_ret(old_portfolio_weight_series, new_portfolio_weight_series, buy_date, sell_date):
    ...
    cost = (old_new.iloc[:, 0] - old_new.iloc[:, 1]).abs().sum() * TRANSACTION_COST
...
def normalize_series(series):
    ...
    # 对于极端值使用对数变换
    if series.abs().max() > EXTREME_VALUE_THRESHOLD:
        series = np.sign(series) * np.log2(1. + series.abs())

    # 处理无效数据
    if np.isnan(series.mean()) or np.isnan(series.std()) or (series.std() < MIN_STD): 
        series.iloc[:] = 0.
        return series

    # 对于低基数因子直接标准化
    if len(series.unique()) <= LOW_CARDINALITY_THRESHOLD: 
        series = (series - series.mean()) / series.std()
        return series.fillna(0)

    # Winsorization：对极端值进行截尾处理
    q = series.quantile(WINSORIZE_QUANTILES)
...
def train_improved_model():
    ...
    # 训练数据使用start_date之前的数据
    training_dates = get_buy_dates(
        start_date=start_date - datetime.timedelta(365*TRAINING_YEARS), 
        end_date=start_date, 
    ...
    print("  - ElasticNet...")
    models['elastic'] = ElasticNet(**ELASTIC_PARAMS)
...
def evaluate_performance():
    ...
    # 获取基准ETF数据
    benchmark_index = get_price([BENCHMARK_ETF], 
                start_date=start_date, 
                end_date=end_date, 
                frequency='daily',
